...
3. **Search Recipes before inventing.** When the `evolver-proxy` MCP server is enabled, call `evolver_recipe_search` then `evolver_recipe_express`. Fall back to `evolver_search_assets` for genes/capsules only when no Recipe matches. Useful fallback signals: `log_error`, `perf_bottleneck`, `test_failure`, `capability_gap`, `user_feature_request`, `deployment_issue`, or `recurring_error`.
